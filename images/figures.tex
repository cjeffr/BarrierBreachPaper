...
\begin{tikzpicture}
    \begin{axis}
        [myplotstyle,
        table/col sep=comma,
        xlabel={Time from hurricane landfall (hrs)},
        label={Surge height (m)}]
        \addplot+[]table[x=Time, y=0, col sep=comma]{../data/depth_gauge0011_east.csv};
        % \foreach \i in {depth, width, loc, no_breach, wd}{
        %     \addplot+table[x=Time, y=0, col sep=comma]{data/\i_gauge0011_east.csv};}
        % \addlegendentry[options]{Mean \i}}
    \end{axis}
\end{tikzpicture}
...
% \edef\temp{\noexpand\addlegendentry{{\i} breaches}}
% \temp
}
\end{axis}
\end{tikzpicture}
